...
class Store {
  ...
  @computed get origem(): estado[] { return this.estados.filter((estado) => (estado.UF !== this.dados.dest || estado.UF === '')); };
  @computed get destino(): estado[] { return this.estados.filter((estado) => (estado.UF !== this.dados.orig || estado.UF === '')); };
